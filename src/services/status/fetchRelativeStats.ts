...
export function fetchRelativeStats(params: IFetchRelativeStatsParams): Promise<IParsedStats[]> {
  ...
  return Promise.all(requests)
    .then(all => all.reduce<IParsedStats[]>((result, item, index) => {
      if (index === 0) {
        return [
          ...item.filter(({ date }) => all.length === 1
      ...
      if (index === all.length - 1) {
        return [
          ...result,
          ...item.filter(({ date }) => to.isAfter(date)),
        ];
      }
